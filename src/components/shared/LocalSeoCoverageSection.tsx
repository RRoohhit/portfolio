...
import { LOCAL_COVERAGE, WHY_HIRE, SEO_KEYWORD_CITIES } from "@/data/portfolioData";
...
export const LocalSeoCoverageSection: React.FC<LocalSeoCoverageSectionProps> = ({ onContact }) => {
  return (
    <section id="local-seo-coverage" className="space-y-8 scroll-mt-24" aria-label="SEO specialist coverage across India">
      
      {/* Heading Block — keyword-rich for 'SEO specialist near me', Hallo, Delhi & India */}
      <div className="bg-zinc-950 border border-white/10 p-6 sm:p-8 rounded-3xl relative overflow-hidden shadow-2xl">
        ...
        <div className="space-y-3 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[10px] font-mono uppercase tracking-widest font-bold">
            ...
            <span>SEO Specialist Near Me — Noida • Delhi • India</span>
          </div>
          <h2 className="text-2xl sm:text-4xl font-black text-white tracking-tight uppercase">
            Local SEO Services & Data-Driven Growth Across <span className="text-emerald-400">Noida, Delhi &amp; All India</span>
          </h2>
          <p className="text-xs sm:text-sm text-white/70 leading-relaxed">
            Looking for an <strong className="text-white font-bold">SEO specialist near me</strong>, an <strong className="text-white font-bold">SEO expert in Noida or Delhi</strong>, or a nationwide <strong className="text-white font-bold">SEO consultant in India</strong>? Rohit Gupta works with businesses in every city below and remotely anywhere in India — delivering Rank #1 Google results, AI search citations, and sales-ready organic traffic.
          </p>
        </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 text-xs font-mono text-white/60 uppercase tracking-wider">
          ...
          <span>Where I Work & The 'near me' SEO Keywords I Rank For</span>
        </div>
        <h3 className="text-xl sm:text-3xl font-extrabold text-white tracking-tight mt-1">
          Hire the Best SEO Specialist in Your City
        </h3>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {LOCAL_COVERAGE.map((item, idx) => (
          <div
            key={idx}
            className="p-5 rounded-2xl bg-white/5 border border-white/10 hover:border-emerald-500/40 hover:bg-white/[0.07] transition-all duration-300 space-y-3 flex flex-col"
          >
            ...
            <div className="flex flex-wrap gap-1.5 pt-1 mt-auto">
              {item.tags.map((tag, tIdx) => (
                <span key={tIdx} className="px-2 py-1 rounded-lg bg-black border border-white/10 text-[9px] font-mono text-white/60">
                  #{tag}
                </span>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="p-6 rounded-2xl bg-white/5 border border-white/10 space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-white border border-white/20 text-[10px] font-mono uppercase tracking-widest">
            ...
            <span>Roles Companies Hire For</span>
          </div>
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {["SEO Specialist", "SEO Executive", "Technical SEO Consultant", "SEO Analyst & Auditor", "Full Stack Web Developer + SEO", "SEO Manager (Virtual)", "E-Commerce SEO Expert", "Local SEO & GMB Specialist"].map((role) => (
        ...
        <div className="p-6 rounded-2xl bg-gradient-to-br from-emerald-950/60 to-zinc-950 border border-emerald-500/20 space-y-4 flex flex-col justify-between">
          <div className="space-y-2">
            <h3 className="text-lg sm:text-2xl font-extrabold text-white tracking-tight">
              Looking for a Rank #1 SEO expert near Me?
            </h3>
            <p className="text-xs text-white/75 leading-relaxed font-light">
              Get a free 30-minute SEO consultation and a free 14-point technical audit for your website. I'll show you exactly how to outrank your competitors in Noida, Delhi, NCR and beyond.
            </p>
            <div className="flex flex-wrap gap-1.5 pt-1">
              {SEO_KEYWORD_CITIES.slice(0, 8).map((kw, idx) => (
                <span key={idx} className="px-2 py-1 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-[9px] font-mono text-emerald-300">
                  #{kw}
                </span>
              ))}
            </div>
          ...
          >
            ...
            <span>Contact SEO Specialist</span>
